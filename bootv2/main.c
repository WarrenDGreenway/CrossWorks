...
void taskVoltage( void )
{
    ...
    //voltage = adcConvertRawToFloat( pinAnalog( PA1) );
    //print( STDIO, "\rVoltge: %f", voltage );
    raw = adcSingleSample( VOLTAGE_IN );
    voltage = adcConvertRawToFloat( raw );

    print( STDIO, "\rVoltage Float: %1.2f", voltage );
    print( STDIO, "Voltage Raw: %d", raw );
    //print( STDIO, "Voltage: %d", adcSingleSample( VOLTAGE ) );
}
...
{   
    ...
    */
    ...
    pinConfig( VOLTAGE_IN, ANALOG );
    pinConfig( CURRENT_IN, ANALOG );
    adcEnableChannel( VOLTAGE_IN );
    adcEnableChannel( CURRENT_IN );
    adcSingleSampleInit();
    taskNewSimple( "volt", 100, (PYGMYFUNCPTR)taskVoltage );
    //convertFloatToString( 3.3, "%1.2", floatString );
    //print( STDIO, "%1.2f", 3.3 );//floatString );
    //taskVoltage();
    while( 1 ){
        // Wait for commands
    }
